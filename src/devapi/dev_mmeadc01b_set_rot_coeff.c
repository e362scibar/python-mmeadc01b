 ...
 * @param    [in]   lsb    uint32_t ::= LSb for rotation coefficient.
 * @return          stat    int     ::= process status
 */
int
_mmeadc01b_calc_iq_rot_coeff(double gain, trigfunc trig, double rad, uint32_t *wdat, uint32_t lsb)
{
...
_mmeadc01b_set_rot_coeff_iq                    (int fd, int    ch, double gain, double rad)
{
    ...
    int       num       = 1;
    uint32_t  wdat;
    uint32_t  lsb       = MMEADC01B_IQ_ROT_COEFF_LSB;
    ...
    stat     = _mmeadc01b_calc_iq_rot_coeff(gain, cos, rad, &wdat, lsb);
    if (stat <  0) {
        return  stat;
    }
    stat     =  dev_mmeadc01b_write(fd, id_bar, ofs, num, &wdat);
    ...
    stat     = _mmeadc01b_calc_iq_rot_coeff(gain, sin, rad, &wdat, lsb);
    if (stat <  0) {
        return  stat;
    }
    stat     =  dev_mmeadc01b_write(fd, id_bar, ofs, num, &wdat);
